...
double testDPF()
{
    ...
    struct DPFKey *kA = malloc(SUMT * sizeof(struct DPFKey));
    struct DPFKey *kB = malloc(SUMT * sizeof(struct DPFKey));

    for (size_t i = 0; i < SUMT; i++)
        DPFGen(prf_keys, size, secret_index, secret_msg, msg_len, &kA[i], &kB[i]);
